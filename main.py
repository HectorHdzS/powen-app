...
# Esta es la nueva línea que pone el logo en la esquina superior izquierda
# Asegúrate de que el archivo "logo.png" esté en la misma carpeta que este script
st.logo("logo.png")
...
# --- SECCIÓN: REGISTRO ---
    ...
    with st.form("form_registro", clear_on_submit=True):
        ...
        ubicacion = col1.selectbox("Ubicación", sorted(list(COORDENADAS.keys())))
        ...
        if st.form_submit_button("💾 Guardar en Nube"):
            if nombre and potencia > 0:
                with conn.session as s:
                    ...
                    s.commit()
                st.success(f"✅ Proyecto '{nombre}' guardado en Neon (Nube).")
                st.rerun()
...
# --- SECCIÓN: MAPA ---
    ...
    if not df_mapa.empty:
        ...
        st.warning("No hay datos en el mapa.")
